...
void ShapesDrawingLayer::DrawEngineObjects()
{
...
void ShapesDrawingLayer::DrawMapVectorShapes()
{
    ...
    for(Layer *l : shapesMap->GetLayers()){
...
Sprite * ShapesDrawingLayer::DrawSpriteCollisionShapes(Sprite *sprite, int &countDrawn)
{
    ...
    if(sprite->GetKind()==SpriteKind::STANDARD){
        ...
        for(VectorShape * vs : standardSprite->GetActiveImage()->GetExtraShapes()) allShapes.push_back(vs);
        ...
        //if(standardSprite->PointInside(GetMap()->GetCamera()->MapPointFromScreenPoint(sceneCursor.GetScreenPosition()))){
        if(standardSprite->PointInside(GetMap()->GetCamera()->MapPointFromScreenPoint(Vec2iToVec2f(mouse.GetPos())))){
