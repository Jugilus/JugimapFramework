...
GraphicFile * JugiMapBinaryLoader::LoadGraphicFile(BinaryStreamReader &stream, int size)
{
    ...
    while(sizeCounter>0){
        ...
        if(signature==SaveSignature::VARIABLES){
            ...
            }else if(fileKind==jmTEXT_OBJECT){
                gf->kind = FileKind::SPRITE_TEXT_CONTAINER;
            }
            ...
            for(int i=0; i<nItems; i++){
                ...
                if(signature2==SaveSignature::SOURCE_IMAGE){
                    GraphicItem *gi = LoadGraphicItem(stream, sizeChunk2);
                    if(gi){
                        gi->graphicFile = gf;
                        gf->items.push_back(gi);
                    }

                }else{
                    messages.push_back("LoadSourceSprite - SOURCE_IMAGE - unknown signature: " + std::to_string(signature2));
                    stream.Seek(posChunk2+sizeChunk2);
                }
...
GraphicItem * JugiMapBinaryLoader::LoadGraphicItem(BinaryStreamReader &stream, int size)
{
    ...
    if(settings.IsYCoordinateUp()){
        im->handle.y = im->rect.GetHeight() - im->handle.y;
    }

    // text label
    int tl = stream.ReadByte();
    if(tl==1){
        im->fontStyleId = stream.ReadInt();
        im->colorStyleId = stream.ReadInt();
        int idHandle = stream.ReadInt();
        im->textHandleVariant = GetTextHandleVariantFromInt(idHandle);
        im->label = stream.ReadString();
    ...
    for(int k=0; k<nCollisionShapes; k++){
        ...
        if(signature2==SaveSignature::VECTOR_SHAPE){
            ...
            messages.push_back("LoadGraphicItem - VECTOR_SHAPE - unknown signature: " + std::to_string(signature2));
            stream.Seek(posChunk2+sizeChunk2);
        }
    }
    InitCollisionShapes(im);
    ...
    while(sizeCounter>0){
        ...
        if(signature==SaveSignature::VARIABLES){
            ...
            for(int i=0; i<nItems; i++){
                ...
                //}else if(indexGraphicFile==saveID_DUMMY_SIF_TEXT_SPRITE){
                //    ss->kind = SpriteKind::TEXT;
                    ...
                    if(indexGraphicItem>=0 && indexGraphicItem<sif->items.size()){
                        ...
                        }else if(sif->kind==FileKind::SPRITE_TEXT_CONTAINER){
                            ss->kind = SpriteKind::TEXT;
...
bool JugiMapBinaryLoader::_LoadMap(BinaryStreamReader &stream, int size)
{
    ...
    while(sizeCounter>0){
        ...
        if(signature==SaveSignature::VARIABLES){
            ...
            map->boundingBox.min.x = stream.ReadInt();
            map->boundingBox.min.y = stream.ReadInt();
            map->boundingBox.max.x = stream.ReadInt();
            map->boundingBox.max.y = stream.ReadInt();
...
bool JugiMapBinaryLoader::LoadSpriteData(BinaryStreamReader &stream, Sprite *s)
{
    ...
    const int saveSOURCE_OBJECT_REPLACEMENT = 1 << 13;
    const int saveCHILD_SOURCE_OBJECT_REPLACEMENTS = 1 << 14;
    ...
    if(saved & saveSOURCE_OBJECT_REPLACEMENT){
        int indexSourceSprite = stream.ReadInt();
        indexSourceSprite += indexBeginSourceSprites;
        if(indexSourceSprite<0 || indexSourceSprite>=sourceGraphics->sourceSprites.size()){
            error = "LoadSpriteData source object replacement indexSourceSprite: " + to_string(indexSourceSprite) + " out of range!";
            return false;
        }
        s->ssr = new SourceSpriteReplacements(sourceGraphics->sourceSprites[indexSourceSprite], "", s->sourceSprite);
    }

    if(saved & saveCHILD_SOURCE_OBJECT_REPLACEMENTS){
        int nChilds = stream.ReadInt();
        for(int i=0; i<nChilds; i++){
            int indexSourceSprite = stream.ReadInt();
            std::string childSpriteName = stream.ReadString();

            if(s->ssrChildren==nullptr){
                s->ssrChildren = new SourceSpriteReplacements(sourceGraphics->sourceSprites[indexSourceSprite], childSpriteName, s->sourceSprite);
            }else{
                s->ssrChildren->replacements.push_back(SourceSpriteReplacement(sourceGraphics->sourceSprites[indexSourceSprite], childSpriteName));
            }
        }
    }
...
void JugiMapBinaryLoader::InitCollisionShapes(GraphicItem *gi)
{
    ...
    */
    ...
    for(VectorShape *vs : gi->extraShapes) allShapes.push_back(vs);
