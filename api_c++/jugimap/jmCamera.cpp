...
void WorldMapCamera::UpdateTransformation()
{
    ...
    if(constrainedInScrollableRect){
        Vec2f mapNoScrollingBorder((viewport.Width()/2.0)/scale, (viewport.Height()/2.0)/scale);
    ...
    AffineMat3f m = MakeTransformationMatrix(-viewport.Center(), Vec2f(scale,scale), Vec2i(), rotation);
